...
class MessagesModelMessages extends JModel
{
	...
	function _buildQuery()
	{
		...
		$query = 'SELECT n, idFrom, subject, message, date, seen FROM #__messaging WHERE idTo='.$user->id.' ORDER BY date DESC';
		return $query;
	}
	...
	*/
	function getData()
	{
		...
		for($i = 0; $i < sizeof($this->_data); $i++){
			...
			$this->_data[$i]->date = $this->transformDate($this->_data[$i]->date);
		}
		
		//Put all messages to "seen"
		$db =& JFactory::getDBO();
		$thisuser =& JFactory::getUser();
		$db->setQuery('UPDATE #__messaging SET seen=1 WHERE idTo='.$thisuser->id.' AND seen=0');
		$db->query();
